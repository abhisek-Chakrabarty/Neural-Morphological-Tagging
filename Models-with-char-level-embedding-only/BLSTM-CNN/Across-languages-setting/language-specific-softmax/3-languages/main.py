...
train_data_class_annotation_1, train_data_class_annotation_2, train_data_class_annotation_3 = ld.load_class_annotation_from_train_data()
...
print ('train_data_class_annotation_3.shape: ' + str(train_data_class_annotation_3.shape))
no_of_classes_1 = int(train_data_class_annotation_1.shape[1])
no_of_classes_2 = int(train_data_class_annotation_2.shape[1])
no_of_classes_3 = int(train_data_class_annotation_3.shape[1])
...
preprocessing.Preprocessing(train_file).pad_data(train_data_class_annotation_3, padding_dictionary, train_file + '_class_annotation_3', divide_file_factor)
reshape_size_1 = int(train_data_class_annotation_1.shape[1])
reshape_size_2 = int(train_data_class_annotation_2.shape[1])
reshape_size_3 = int(train_data_class_annotation_3.shape[1])
del train_data_class_annotation_1, train_data_class_annotation_2, train_data_class_annotation_3
gc.collect()
...
train_data_class_annotation_3 = ld.load_padded_data(reshape_size_3, train_file + '_class_annotation_3')
...
print ('After Padding train_data_class_annotation_3.shape is : ' + str(train_data_class_annotation_3.shape))
...
train_data_class_annotation_3 = train_data_class_annotation_3.reshape(-1, max_sentence_length, int(train_data_class_annotation_3.shape[1]))
...
tag_classify_model = models.get_tag_classify_model_with_CNN(max_sentence_length+2*filter_window_length, 2*char_feature_output, no_of_kernels, filter_window_length, no_of_classes_1, no_of_classes_2, no_of_classes_3)
...
if os.path.isfile(os.path.join('./', tag_classify_model_file_name)):
    ...
    tag_classify_model.fit([train_char_vectors], [train_data_class_annotation_1, train_data_class_annotation_2, train_data_class_annotation_3], nb_epoch=nb_epoch, batch_size=batch_size, )
...
del train_char_vectors, train_data_class_annotation_1, train_data_class_annotation_2, train_data_class_annotation_3
gc.collect()
...
output_1, output_2, output_3 = tag_classify_model.predict([test_char_vectors])
...
output_3 = output_3.reshape(int(output_3.shape[0])*int(output_3.shape[1]), -1)
...
with io.open(train_file, 'r', encoding='utf-8') as f1:
    input_lines = f1.readlines()
f1.close()
lan_id_dic = {}
id = 1
...
for i in range(0, len(output_1)):
    ...
    max_prob = -1.0;
    index = -1
    for j in range(0, len(output_3[i])):
        if max_prob < output_3[i][j]:
            max_prob = output_3[i][j]
            index = j
    output_tag_classes_3.append(index)
...
class_to_tag_dictionary_file_3 = open(train_file + '_class_to_tag_mapping_3', "rb")
...
class_to_tag_dictionary_3 = cPickle.load(class_to_tag_dictionary_file_3)
class_to_tag_dictionary_file_1.close()
class_to_tag_dictionary_file_2.close()
class_to_tag_dictionary_file_3.close()
...
with io.open(test_file, 'r', encoding="utf-8") as f:
    for line in f:
        ...
        elif lan_id_dic[char] == 2:
            predicted_tag = class_to_tag_dictionary_2[int(output_tag_classes_2[i]) + 1]
        else:
            predicted_tag = class_to_tag_dictionary_3[int(output_tag_classes_3[i]) + 1]
        i += 1
        if len(fields) >= 2:
            evaluation_flag = 1
