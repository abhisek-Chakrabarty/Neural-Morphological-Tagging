...
tag_classify_model = models.get_tag_classify_model(tag_classify_network_cell, max_sentence_length, 2*char_feature_output, hidden_size, no_of_classes)
...
if os.path.isfile(os.path.join('./', tag_classify_model_file_name)):
    ...
    tag_classify_model.fit([train_char_vectors], [train_data_class_annotation], nb_epoch=nb_epoch, batch_size=batch_size, validation_split=0.2)
    #history = np.array(history)
    #np.savetxt("history.txt", history, delimiter=",")
